...
extension UIColor {
    ...
    convenience init?(hex:String) {
        ...
        let start = hex.index(hex.startIndex, offsetBy: result.rangeAt(1).length + result.rangeAt(1).location)
        
        
        //let start = hex.startIndex.advancedBy(result.rangeAt(1).length + result.rangeAt(1).location) //advance(hex.startIndex, result.rangeAtIndex(1).length + result.rangeAtIndex(1).location)
